...
%y is a 5000x1 array with values ranging from 1:10. 
%we need to convert this to a 10x5000 array
yy = zeros(size(a3,1),m);
for i = 1:K
  for j = 1:m
    yy(i,j) = (y(j)==i);
  end
...
Jmatrix = -1*(yy.*log(a3)+(1-yy).*log(1-a3))/m;
...
Theta1_copy(:,1) = 0;
Theta2_copy(:,1) = 0;
regularizationCost = lambda*(sum(sum(Theta1_copy.^2))+sum(sum(Theta2_copy.^2)))/(2*m);
